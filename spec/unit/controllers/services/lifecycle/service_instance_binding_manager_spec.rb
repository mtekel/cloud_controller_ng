  ...
  describe ServiceInstanceBindingManager do
    ...
    describe '#create_route_service_instance_binding' do
      ...
      it 'creates a binding' do
        expect(route.route_binding).to be_nil
        expect(service_instance.route_bindings).to be_empty

        manager.create_route_service_instance_binding(route, service_instance)

        route_binding = RouteBinding.last
        expect(route.reload.route_binding).to eq route_binding
        expect(service_instance.reload.route_bindings).to include route_binding
      end
      ...
      context 'and the bind request returns a syslog drain url' do
        ...
        it 'does not create a binding and raises an error for services that do not require syslog_drain' do
          ...
          expect(route.reload.route_binding).to be_nil
          expect(service_instance.reload.route_bindings).to be_empty
        end
    ...
    describe '#delete_route_service_binding' do
      ...
      it 'deletes the binding and removes associations from routes and service_instances' do
        expect(route.route_binding).to eq route_binding
        expect(service_instance.route_bindings).to include route_binding
