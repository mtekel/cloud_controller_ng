...
module VCAP::CloudController
  class AppCreateMessage < BaseMessage
    ...
    def initialize(params)
      super
      if space_guid
        requested_keys.concat [:space, :guid]
        relationships['space'] ||= relationships[:space]
        relationships['space']['guid'] ||= relationships[:space][:guid]
      end
    end

    def allowed_keys
      [:name, :relationships, :environment_variables, :buildpack, :space_guid]
    end
